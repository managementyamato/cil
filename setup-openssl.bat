...
echo.
echo 3. 以下のファイルをこのフォルダにコピー:
echo    [解凍フォルダ]\ext\php_openssl.dll  → %cd%\ext\php_openssl.dll
echo    [解凍フォルダ]\ext\php_curl.dll     → %cd%\ext\php_curl.dll
echo    [解凍フォルダ]\libcrypto-3-x64.dll  → %cd%\libcrypto-3-x64.dll
echo    [解凍フォルダ]\libssl-3-x64.dll     → %cd%\libssl-3-x64.dll
echo.
...
echo ----------------------------------------
echo.
echo   php.exe -m | findstr /i "openssl curl"
echo.
