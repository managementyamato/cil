            ...
            <<?= $tagName ?> class="st-product-card<?= $hasUrl ? ' is-clickable' : '' ?>"
                ...
                <div class="st-product-visual" aria-hidden="true">
                    <?php if (!empty($p['icon_image'])): ?>
                        <img src="<?= htmlspecialchars($p['icon_image']) ?>" alt="" style="width:42px;height:42px;object-fit:contain">
                    <?php elseif (!empty($p['icon'])): ?>
                        <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><?= $p['icon'] ?></svg>
                    <?php else: ?>
