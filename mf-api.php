...
class MFApiClient {
    ...
    public function handleCallback($code, $redirectUri) {
        ...
        if (function_exists('curl_init')) {
            ...
            if ($response === false) {
                throw new Exception('トークン取得エラー: HTTPリクエストが失敗しました - ' . $curlError);
            }
        } else {
            // cURLが使えない場合はfile_get_contentsを使用（ローカル環境用）
            $options = array(
                'http' => array(
    ...
    public function refreshAccessToken() {
        ...
        if (function_exists('curl_init')) {
            ...
            if ($response === false) {
                throw new Exception('トークンのリフレッシュに失敗しました - ' . $curlError);
            }
        } else {
            // cURLが使えない場合はfile_get_contentsを使用（ローカル環境用）
            $options = array(
    ...
    private function request($method, $endpoint, $data = null) {
        ...
        if (function_exists('curl_init')) {
            ...
            if ($response === false) {
                throw new Exception('APIリクエスト失敗: HTTPリクエストが失敗しました - ' . $curlError);
            }
        } else {
            // cURLが使えない場合はfile_get_contentsを使用（ローカル環境用）
