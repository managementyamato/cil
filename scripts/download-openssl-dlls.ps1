...
if (-not (Test-Path $libDir)) {
    Write-Host "libフォルダを作成中..." -ForegroundColor Green
    New-Item -ItemType Directory -Path $libDir | Out-Null
    Write-Host "✓ libフォルダを作成しました" -ForegroundColor Green
}
...
try {
    ...
    $filesToCopy = @(
        ...
        @{
            ...
            Dest = "$libDir\libcrypto-3-x64.dll"
            Name = "libcrypto-3-x64.dll"
        },
        @{
            Source = "$tempExtract\libssl-3-x64.dll"
            Dest = "$libDir\libssl-3-x64.dll"
