...
REM プロジェクトルートディレクトリに移動（scriptsの親ディレクトリ）
cd /d "%~dp0.."
...
if exist "%~dp0php.exe" (
    set "PHP_CMD=%~dp0php.exe"
    echo [OK] リポジトリ内のポータブルPHPを使用します (scripts)
    "%PHP_CMD%" -v | findstr /C:"PHP"
    echo.
    goto :php_found
)

if exist "%~dp0..\lib\php.exe" (
    set "PHP_CMD=%~dp0..\lib\php.exe"
    echo [OK] リポジトリ内のポータブルPHPを使用します (lib)
    "%PHP_CMD%" -v | findstr /C:"PHP"
    echo.
    goto :php_found
)
...
echo   - %~dp0..\lib\php.exe (推奨: ポータブル版)
