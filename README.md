...
- [x] 月別集計ページの実装
- [x] デバッグ機能の実装
- [x] CLIベースの最新データ取得スクリプト（fetch-latest-data.php）
...
MFから最新の請求書データを取得するには、2つの方法があります:

#### 方法1: Webインターフェースから（推奨）

1. ブラウザで `finance.php` にアクセス
2. 「MFから同期」ボタンをクリック
3. 自動的に最新データが取得・更新されます
...
**自動同期:**
- デフォルトでは無効（手動同期のみ）
- 有効にする場合は `finance.php` の `$autoSyncEnabled = true` に変更
